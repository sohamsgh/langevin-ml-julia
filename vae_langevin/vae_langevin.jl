...
# load images data and return loader
...
function get_pv_data(batch_size)
    data = readdlm("pv-baobab.txt", Float32) |> transpose
    xtrain = data
    ytrain = circshift(xtrain, (0,1))
    return DataLoader((xtrain, ytrain), batchsize=batch_size, shuffle=true)
end
...
function train(; kws...)
    ...
    # load pv data
    loader = get_pv_data(args.batch_size)
    ...
    # fixed input
    original, _ = first(get_pv_data(args.sample_size))
    original = original |> device
    #output_path = joinpath(args.save_path, "original.txt")
    #save(output_path, original)
    # training
    train_steps = 0
    ...
    for epoch = 1:args.epochs
        ...
        output_path = joinpath(args.save_path, "epoch_$(epoch).txt")
        #save(output_path, rec_original)
        @info "output not saved: $(output_path)"
    end
    exit() 
